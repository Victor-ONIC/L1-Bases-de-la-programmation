...
void coutPersonne(personne p);
int whichWeekDay(int d, int m, int y);
date createNewDate(int a, int b);
date createDateFromArray(char T[]);
bool isPrior(date d1, date d2);
bool isEqual(date d1, date d2);
personne createNewPersonne();
...
int main() {
  ...
  for (int i = 0; i < N; i++) T[i] = createNewDate(1950, 2021);  // entre 1950 et 2021
...
int whichWeekDay(int d, int m, int y) {  // 0: Dimanche, 1: Lundi, ...
  ...
  fDay = d;  // fDay
  if (m == 1)  // fMonth les mois commencent par 1.Mars, ...
    fMonth = 11;
  else if (m == 2)
    fMonth = 12;
  else
    fMonth = m - 2;
  if (y > 1999) {  // fYear & fCentury ex: 2019 -> fYear:19, fCentury:20
...
personne createNewPersonne() {  // créer une personne
  ...
  char NOMS[30][30] = {
    ...
    {"Michel"},        {"Ouahw"},   {"Aime"},
    {"Haais"},         {"Pedet"},   {"Ayamget Enndailaikitt"},
    {"Zouzoua"},       {"Biz"},     {"Feuques"}
  };
